export function injectSearch(html: string): {
  ...
  const processedHtml = html.replace(
    ...
    (_match, attributes = "") => {
      ...
      const buttonText =
        attributes.match(/button=["']([^"']*)["']/i)?.[1] ??
        "Search";

      // Shortcode attributes are author supplied, so never place them in HTML
      // without escaping. More attributes can be added here as supported.
      ...
      return `
        <div class="cms-search" data-cms-search>
