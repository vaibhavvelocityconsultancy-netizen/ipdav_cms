...
export async function getAllSearches(tenantId) {
  return prisma.searchConfiguration.findMany({
    where: { tenantId },
...
export async function createSearch(input, tenantId) {
  return prisma.searchConfiguration.create({
    data: { ...input, customClass: cleanClass(input.customClass), tenantId },
  });
}

export async function updateSearch(id, input, tenantId) {
  return prisma.searchConfiguration.updateMany({
    where: { id, tenantId },
    data: { ...input, customClass: cleanClass(input.customClass) },
  });
}
