...
export default function PricingPlansPage() {
  ...
  const plans = useMemo(() => {
    ...
    return allPlans.filter((p: Plan) => {
      if (selectedCycle === "MONTHLY") return p.allowMonthly && p.monthlyPrice != null;
      return p.allowYearly && p.yearlyPrice != null;
    });
  }, [allPlans, showToggle, selectedCycle]);

  async function handleSelectPlan(plan: Plan, billingCycle: "MONTHLY" | "YEARLY") {
    ...
    if (!currentUser) {
      const target = plan.trialDays && plan.trialDays > 0
  ...
  function getPlanPrice(plan: Plan, cycle: "MONTHLY" | "YEARLY"): number | null {
    if (cycle === "MONTHLY") return plan.allowMonthly ? plan.monthlyPrice : null;
        ...
        <>
          <section className="w-full bg-[#0B0F1A] py-20">
            <div className="mx-auto max-w-7xl px-6 text-center">
              <h1 className="text-5xl font-bold text-white lg:text-7xl">Pricing</h1>
              <div className="mt-6 flex justify-center">
                <nav className="breadcrumb" aria-label="breadcrumb">
                  <ol className="breadcrumb-list flex items-center flex-wrap text-sm">
                    <li className="flex items-center">
                      <a href="/" className="breadcrumb-link text-slate-400 hover:text-white transition-colors">
                        Home
                      </a>
                  ...
                  <div className="mt-8 inline-flex items-center bg-slate-100 rounded-full p-1">
                    <button
                      ...
                      className={`px-6 py-2 text-sm font-medium rounded-full transition-colors ${
                        selectedCycle === "MONTHLY" ? "bg-slate-900 text-white" : "text-slate-600 hover:text-slate-900"
                      }`}
                    >
                      Monthly
                    </button>
                    <button
                      ...
                      className={`px-6 py-2 text-sm font-medium rounded-full transition-colors ${
                        selectedCycle === "YEARLY" ? "bg-slate-900 text-white" : "text-slate-600 hover:text-slate-900"
                      }`}
                    >
                      Yearly
                    </button>
                  ...
                  return (
                    <div
                      ...
                      className={`relative w-full rounded-none overflow-hidden flex flex-col border-t-4 ${
                        dark ? "bg-slate-900 border-slate-900 text-white" : "bg-slate-50 border-slate-900 text-slate-900"
                      }`}
                    >
                      <div className="p-8 flex flex-col flex-grow">
                        <h3 className={`font-serif text-5xl mb-3 ${dark ? "text-white" : "text-slate-900"}`}>
                          {plan.title}
                        </h3>
                        ...
                        <div className="mb-6 flex items-baseline gap-2">
                          <span className="font-serif text-6xl">{formatUSD(price)}</span>
                          <span className={`text-lg ${dark ? "text-slate-300" : "text-slate-600"}`}>
                            / {label}
                          </span>
                        </div>
                        ...
                        <div className={`border-t mb-6 ${dark ? "border-slate-700" : "border-slate-300"}`} />
                          ...
                          <div className="flex-grow space-y-3">
                            {plan.features.map((f: Feature) => (
                              <div key={f.id} className="flex items-start gap-3">
                                ...
                                <span className={`text-base font-serif ${dark ? "text-slate-100" : "text-slate-800"}`}>
                                  {f.title}
                                </span>
                              </div>
                            ))}
