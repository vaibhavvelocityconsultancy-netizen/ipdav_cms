...
function renderResults(results: HTMLElement, items: SearchItem[]) {
  ...
  items.forEach((item) => {
    ...
    link.href = appUrl(item.type === "post" ? `/posts/${item.slug}` : `/${item.slug}`);
...
/** Handles shortcode HTML inserted via dangerouslySetInnerHTML using delegation. */
export default function PublicSearchShortcodeHandler() {
  useEffect(() => {
    ...
    const onSubmit = async (event: SubmitEvent) => {
      ...
      const button = form.querySelector<HTMLButtonElement>(".cms-search-button");
